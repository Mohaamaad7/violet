...
### د. نظام الإرسال
1. ✅ إرسال مباشر (بدون Queue Worker)
2. ✅ تتبع الحالة لكل بريد (Campaign Logs)
3. ✅ تسجيل حالات الفشل مع سبب الخطأ
4. ✅ تحديث الإحصائيات تلقائياً
5. ✅ تعليم البريد كـ bounced في حالات واضحة (bounce/invalid)
...
### 1. الإرسال المباشر
لا تحتاج لتشغيل Queue Worker. عند الضغط على **Send Now** سيتم إرسال الحملة مباشرة.
...
2. أكد الإرسال
3. سيتم إرسال الحملة مباشرة
4. راقب الإحصائيات وتسجيلات الإرسال
...
### 3. Queue Worker (اختياري)
لو لاحقاً احتجت إرسال أعداد كبيرة بدون أي احتمالية timeout، يمكن تفعيل الإرسال عبر Queue وتشغيل Queue Worker.
...
**الحل**:
1. تأكد من إعدادات البريد (SMTP) صحيحة
2. راجع `storage/logs/laravel.log` لمعرفة سبب الفشل
3. لو عدد المشتركين كبير، قد تحتاج التحويل لإرسال عبر Queue لتجنب timeout
...
2. ⚠️ الإرسال حالياً مباشر؛ في الأعداد الكبيرة قد تحتاج Queue لتجنب timeout
