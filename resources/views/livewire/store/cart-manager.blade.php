...
<div wire:id="cart-manager">
    {{-- Backdrop Overlay --}}
    <div x-show="$wire.isOpen" x-transition:enter="transition-opacity ease-linear duration-300"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" @click="$wire.isOpen = false"
        class="fixed inset-0 bg-black bg-opacity-50 z-40" x-cloak></div>

    {{-- Slide-over Panel --}}
    <div x-show="$wire.isOpen" x-transition:enter="transform transition ease-in-out duration-300"
        x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"
        x-transition:leave="transform transition ease-in-out duration-300" x-transition:leave-start="translate-x-0"
        x-transition:leave-end="translate-x-full" class="fixed inset-y-0 right-0 max-w-full flex z-50" x-cloak>
        <div class="w-screen max-w-md">
            <div class="h-full flex flex-col bg-white shadow-xl">
                {{-- Header --}}
                <div
                    class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-violet-600 to-violet-800 text-white">
                    <div class="flex items-center gap-2">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                        </svg>
                        <h2 class="text-xl font-bold">{{ trans_db('messages.cart_manager.title') }}</h2>
                        @if($cartCount > 0)
                    ...
                    <button @click="$wire.isOpen = false" class="p-1 hover:bg-white/20 rounded-lg transition"
                        title="إغلاق">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex-1 overflow-y-auto px-6 py-4">
                    @if($cart && $cart->items->count() > 0)
                                ...
                                <div class="flex gap-4 p-4 bg-gray-50 rounded-lg" wire:key="cart-item-{{ $item->id }}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <button wire:click="removeItem({{ $item->id }})" wire:loading.attr="disabled"
                                            ...
                                            <svg wire:loading.remove wire:target="removeItem" class="w-4 h-4" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                            <svg wire:loading wire:target="removeItem" class="w-4 h-4 animate-spin" fill="none"
                                                viewBox="0 0 24 24">
                                            ...
                                            <span wire:loading.remove
                                                wire:target="removeItem">{{ trans_db('messages.cart_manager.remove') }}</span>
                                            <span wire:loading
                                                wire:target="removeItem">{{ trans_db('messages.cart_manager.removing') }}</span>
                                        </button>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        @if($cart->items->count() > 1)
                            <button wire:click="clearCart" wire:confirm="هل أنت متأكد من تفريغ السلة؟"
                                ...
                                {{ trans_db('messages.cart_manager.clear_cart') }}
                            </button>
                        @endif
                    @else
                        {{-- Empty Cart State --}}
                        <div class="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 mb-2">
                                {{ trans_db('messages.cart_manager.empty_title') }}
                            </h3>
                            <p class="text-sm text-gray-500 mb-6">{{ trans_db('messages.cart_manager.empty_message') }}</p>
                            <a href="{{ route('products.index') }}" wire:navigate
                                ...
                                {{ trans_db('messages.cart_manager.browse_products') }}
                            </a>
                        </div>
                    @endif
                </div>
                ...
                @if($cart && $cart->items->count() > 0)
                    <div class="border-t border-gray-200 px-6 py-4 bg-gray-50">
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <span
                                class="text-base font-semibold text-gray-700">{{ trans_db('messages.cart_manager.subtotal') }}:</span>
                        ...
                        <p class="text-xs text-gray-500 mb-4 text-center">
                            {{ trans_db('messages.cart_manager.shipping_note') }}
                        </p>
                        ...
                        <div class="space-y-3">
                            <a href="{{ route('checkout') }}" wire:navigate
                                ...
                                {{ trans_db('messages.cart_manager.checkout') }}
                            </a>
