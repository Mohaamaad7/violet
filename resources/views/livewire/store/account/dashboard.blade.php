            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div class="flex items-center">
                    ...
                    <div class="ms-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_spent'], 2) }} {{ __('messages.egp') }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <nav class="divide-y divide-gray-100">
                        <a href="{{ route('account.profile') }}" class="flex items-center px-6 py-4 hover:bg-gray-50 transition-colors">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                            </svg>
                            <span class="ms-3 text-gray-700">{{ __('messages.account.profile') }}</span>
                            <svg class="w-5 h-5 ms-auto text-gray-400 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                            </svg>
                        </a>
                        <a href="{{ route('account.orders') }}" class="flex items-center px-6 py-4 hover:bg-gray-50 transition-colors">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                            </svg>
                            <span class="ms-3 text-gray-700">{{ __('messages.account.orders') }}</span>
                            @if($stats['pending_orders'] > 0)
                                <span class="ms-auto me-2 px-2 py-0.5 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">
                        ...
                        <a href="{{ route('account.addresses') }}" class="flex items-center px-6 py-4 hover:bg-gray-50 transition-colors">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                            </svg>
                            <span class="ms-3 text-gray-700">{{ __('messages.account.addresses') }}</span>
                            <span class="ms-auto me-2 px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-600 rounded-full">
                                {{ $addressesCount }}
                            </span>
                            <svg class="w-5 h-5 text-gray-400 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <a href="{{ route('account.orders') }}" class="text-sm text-violet-600 hover:text-violet-800 font-medium">
                    ...
                    @if($recentOrders->isEmpty())
                        <div class="p-12 text-center">
                            ...
                            <a href="{{ route('products.index') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors">
                    ...
                    @else
                        <div class="divide-y divide-gray-100">
                            @foreach($recentOrders as $order)
                                <a href="{{ route('account.orders.show', $order) }}" class="block p-6 hover:bg-gray-50 transition-colors">
                                    <div class="flex items-center justify-between mb-3">
                                        <div>
                                            <span class="font-semibold text-gray-900">#{{ $order->order_number }}</span>
                                            <span class="text-sm text-gray-500 ms-2">{{ $order->created_at->format('M d, Y') }}</span>
                                        </div>
                                        @php
                                            $statusColors = [
