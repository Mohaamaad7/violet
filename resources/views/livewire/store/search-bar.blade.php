<div class="relative w-full" x-data="{ 
    ...
    <div class="relative">
        <input type="text" wire:model.live.debounce.300ms="search"
            ...
            class="w-full px-4 py-2.5 {{ $isMobile ? 'pl-10' : 'ltr:pl-10 rtl:pr-10 ltr:pr-10 rtl:pl-10' }} border-2 border-gray-200 rounded-lg focus:border-violet-500 focus:ring-2 focus:ring-violet-200 focus:outline-none transition"
            autocomplete="off">

        {{-- Search Icon --}}
        <svg class="w-5 h-5 absolute {{ app()->getLocale() === 'ar' ? 'right-3' : 'left-3' }} top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
        ...
        @if($search)
            <button type="button" wire:click="clearSearch"
                class="absolute {{ app()->getLocale() === 'ar' ? 'left-3' : 'right-3' }} top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition">
        ...
        <div wire:loading wire:target="search"
            class="absolute {{ app()->getLocale() === 'ar' ? 'left-10' : 'right-10' }} top-1/2 -translate-y-1/2">
