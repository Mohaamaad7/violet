    ...
    <div class="max-w-md mx-auto">
        <div class="text-center mb-8">
            ...
            <p class="mt-2 text-sm text-gray-600">
                {{ __('newsletter.unsubscribe_sorry') }}
            </p>
        </div>
        ...
        <div class="bg-white shadow-md rounded-lg p-8">
            <div class="mb-6">
                <p class="text-gray-700 mb-2">
                    {{ __('newsletter.unsubscribe_email') }}: <strong>{{ $subscriber->email }}</strong>
                </p>
                <p class="text-sm text-gray-500">
                    {{ __('newsletter.subscribed_on') }}: {{ $subscriber->subscribed_at->format('d M Y') }}
                </p>
            </div>
            ...
            <form action="{{ route('newsletter.unsubscribe.process', $subscriber->unsubscribe_token) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="reason" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('newsletter.unsubscribe_reason_question') }} {{ __('newsletter.reason_optional') }}
                    </label>
                    <textarea 
                        name="reason" 
                        id="reason" 
                        ...
                        placeholder="{{ __('newsletter.feedback_helps') }}"
                    ></textarea>
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    >
                        {{ __('newsletter.unsubscribe_button') }}
                    </button>
