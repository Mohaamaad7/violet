...
<div class="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 group relative border border-gray-100"
    ...
    <a href="{{ route('product.show', $product->slug) }}"
        ...
        @if($product->is_on_sale)
            <div
                class="absolute top-3 {{ app()->getLocale() === 'ar' ? 'right-3' : 'left-3' }} bg-red-500 text-white px-2 py-0.5 rounded text-xs font-bold z-10">
                -{{ $product->discount_percentage }}%
            </div>
        @endif

        {{-- Quick Actions Overlay - pointer-events-none to not block clicks below --}}
        <div
            class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none">
    ...
    <div class="p-3">
        ...
        @if($product->category)
            <a href="{{ route('category.show', $product->category->slug) }}"
                class="text-[11px] text-gray-400 hover:text-violet-600 transition uppercase tracking-wide">
        ...
        <h3
            class="font-medium text-gray-800 text-sm mt-1 mb-2 line-clamp-2 leading-snug hover:text-violet-600 transition">
            <a href="{{ route('product.show', $product->slug) }}">
                {{ $product->name }}
            </a>
        </h3>
        ...
        {{-- Actions Row --}}
        <div class="flex items-center gap-2" x-data="{ adding: false }"
            ...
            @else disabled @endif :disabled="adding"
                ...
                <svg x-show="adding" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24">
