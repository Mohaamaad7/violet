...
<div x-data="offersGrid(@js($offers))" id="offers-grid">
    ...
    <div x-show="visibleOffers.length > 0" 
         ...
         class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 transition-all duration-500">
        
        <template x-for="offer in visibleOffers" :key="offer.id">
            {{-- Vertical Offer Card --}}
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group w-full">
                {{-- Image Top --}}
