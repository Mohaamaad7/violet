...
>
    @php
        $product = $getRecord();
        $images = $product ? $product->images()->orderBy('order')->get() : collect();
    @endphp

    <div x-data="{
        images: @js($images->toArray()),
        editingId: null,
        uploading: false,
        ...
        async uploadImages(files) {
            ...
            try {
                let response = await fetch('{{ route('admin.products.upload-images', $product ?? 0) }}', {
                    method: 'POST',
                    body: formData,
                    headers: {
        ...
        async setPrimary(imageId) {
            try {
                let response = await fetch(`{{ url('admin/products/images') }}/${imageId}/set-primary`, {
                    method: 'POST',
                    headers: {
        ...
        async deleteImage(imageId) {
            ...
            try {
                let response = await fetch(`{{ url('admin/products/images') }}/${imageId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content
                    }
        ...
        @if ($product)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 p-6">
                <div class="flex items-center justify-center">
                    <label for="image-upload-{{ $getId() }}" class="cursor-pointer">
