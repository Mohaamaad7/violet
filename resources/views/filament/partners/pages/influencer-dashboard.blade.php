    ...
    <x-layouts.partners :heading="__('messages.partners.dashboard.overview')">
        <div class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div
                    class="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow relative overflow-hidden">
                    <div
                        class="absolute -{{ app()->getLocale() === 'ar' ? 'right' : 'left' }}-4 -top-4 w-24 h-24 bg-primary-50 dark:bg-primary-900/20 rounded-full opacity-50">
                    </div>
                    <div class="flex justify-between items-start relative z-10">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-gray-800 dark:text-gray-200">
                                ...
                                <span class="text-base text-gray-500">{{ __('messages.currency') }}</span>
                            </h3>
                        </div>
                        <div
                    ...
                    <div class="mt-3 flex items-center gap-2 text-xs">
                        ...
                        <span
                            class="text-gray-400 dark:text-gray-500">{{ __('messages.partners.dashboard.growth_from_last_month') }}</span>
                    </div>
                    @if($stats['pending_commission'] > 0)
                        <div
                            class="mt-4 flex items-center text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 w-fit px-2 py-1 rounded-lg">
                            <i class="ph ph-clock m{{ app()->getLocale() === 'ar' ? 'l' : 'r' }}-1"></i>
                            <span class="font-bold">{{ number_format($stats['pending_commission'], 2) }}</span>
                            <span
                                class="text-gray-400 dark:text-gray-500 m{{ app()->getLocale() === 'ar' ? 'r' : 'l' }}-2 text-xs font-normal">
                ...
                <div
                    ...
                    <div class="mt-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full h-1.5">
                        <div class="bg-purple-500 h-1.5 rounded-full"
                            style="width: {{ min(($stats['total_orders'] / 200) * 100, 100) }}%"></div>
                    </div>
                ...
                <div
                    ...
                    <div class="flex justify-between items-start">
                        ...
                        <div
                            class="p-3 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl">
                    ...
                    <div class="mt-2 flex items-center text-sm text-emerald-600 dark:text-emerald-400">
                        <span class="font-medium">
                            {{ __('messages.partners.dashboard.total_paid') }}:
                            {{ number_format($stats['total_paid'], 2) }} {{ __('messages.currency') }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @forelse($discountCodes as $code)
                            <div
                                class="bg-gradient-to-br from-primary-50 to-purple-50 dark:from-primary-900/20 dark:to-purple-900/20 rounded-xl p-4 border border-primary-100 dark:border-primary-900/30 hover:shadow-md transition-shadow">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <span
                                            class="text-lg font-bold text-primary-600 dark:text-primary-400 tracking-wider block">{{ $code->code }}</span>
                                        <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                                            {{ $code->discount_type === 'percentage' ? $code->discount_value . '%' : number_format($code->discount_value, 2) . ' ' . __('messages.currency') }}
                                        </span>
                                    </div>
                                    <span
                                ...
                                @if($code->used_count > 0)
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                                        <i class="ph ph-users"></i> {{ $code->used_count }}
                                        {{ __('messages.partners.dashboard.uses') ?? 'uses' }}
                                    </div>
                                @endif
                                <button x-on:click="navigator.clipboard.writeText('{{ $code->code }}'); 
            ...
            <div
                class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
                <div
                    class="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <h2 class="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center gap-2">
                ...
                @if($recentCommissions->count() > 0)
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead
                                class="bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-xs uppercase font-semibold">
                                <tr>
                                    <th class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }} py-4 px-6">
                                ...
                                @foreach($recentCommissions as $commission)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                        ...
                                        <td class="py-4 px-6 text-gray-600 dark:text-gray-400">
                                            {{ number_format($commission->order_amount, 2) }} {{ __('messages.currency') }}
                                        </td>
