<div x-data="{ 
    ...
    return `<table border=\"1\" cellpadding=\"8\" cellspacing=\"0\" style=\"border-collapse: collapse; width: 100%;\">
        ${this.renderNodes(node.content || [])}</table>`;
    case 'tableRow':
    ...
    return `<${tag}${cellStyle}>${this.renderNodes(node.content || [])}</${tag}>`;
        case 'blockquote':
        return `<blockquote style=\"border-right: 4px solid #ccc; padding-right: 1rem; margin-right: 0;\">
            ${this.renderNodes(node.content || [])}</blockquote>`;
        case 'hardBreak':
        return '<br>';
        default:
        ...
        // Create regex pattern for {{ key }}
        const pattern = self.openBrace + '\\s*' + key + '\\s*' + self.closeBrace;
        const regex = new RegExp(pattern, 'g');
        result = result.replace(regex, self.sampleData[key]);
        });
        return result;
        },
        ...
        <div class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 overflow-hidden">
            ...
            <div
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">معاينة مباشرة للبريد
                    الإلكتروني</span>
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-800">
                <div class="bg-white dark:bg-gray-900 rounded shadow-sm"
                    ...
                    <iframe x-ref="previewFrame" :srcdoc="previewHtml" class="w-full border-0 rounded"
                        ...
                        @load="$refs.previewFrame.style.height = ($refs.previewFrame.contentWindow.document.body.scrollHeight + 40) + 'px'"></iframe>
                </div>
            </div>
