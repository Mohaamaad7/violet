@php
    $record = $getRecord();
    $availableVars = $record?->available_variables ?? [];
    $sampleData = $record ? app(\App\Services\EmailTemplateService::class)->getSampleData($record) : [];
@endphp

<div x-data="livePreviewComponent({{ Js::from($availableVars) }}, {{ Js::from($sampleData) }})" x-init="init()"
    class="fi-fo-field-wrp">
    <div class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 overflow-hidden">
        <!-- Preview Header -->
        <div
            class="px-4 py-3 bg-gray-50 dark:bg-gray-800 border-b border-gray-300 dark:border-gray-600 flex items-center gap-2">
        ...
        <div class="p-4 bg-gray-50 dark:bg-gray-800">
            <div class="bg-white dark:bg-gray-900 rounded shadow-sm"
                ...
                <iframe x-ref="previewFrame" :srcdoc="previewHtml" class="w-full border-0 rounded"
                    style="min-height: 400px;" @load="resizeFrame()"></iframe>
            </div>
        </div>
        ...
        <div class="px-4 py-2 bg-gray-50 dark:bg-gray-800 border-t border-gray-300 dark:border-gray-600">
            <p class="text-xs text-gray-500 dark:text-gray-400">
                💡 <strong>ملاحظة:</strong> المتغيرات يتم استبدالها ببيانات تجريبية للمعاينة فقط.
            </p>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('livePreviewComponent', (availableVariables, sampleData) => ({
            ...
            init() {
                // Watch for content changes via Livewire
                this.$watch('$wire.data.content_html', (value) => {
                    this.rawContent = value;
                });

                // Initial value
                if (this.$wire && this.$wire.data) {
                    this.rawContent = this.$wire.data.content_html || '';
                }
            },
            ...
            replaceVariables(html) {
                ...
                for (let i = 0; i < keys.length; i++) {
                    const k = keys[i];
                    const pattern = self.openBrace + '\\s*' + k + '\\s*' + self.closeBrace;
                    const regex = new RegExp(pattern, 'g');
                    result = result.replace(regex, self.sampleData[k]);
                }
            ...
            resizeFrame() {
                if (this.$refs.previewFrame && this.$refs.previewFrame.contentWindow) {
                    try {
                        this.$refs.previewFrame.style.height = (this.$refs.previewFrame.contentWindow.document.body.scrollHeight + 40) + 'px';
                    } catch (e) {
